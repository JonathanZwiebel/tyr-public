...
public class DrivetrainController implements Requirable {
	...
	public void init() {
		...
		getOutput().getCompressor().automaticMode().on();
		
		reactor.onTriggered(getInput().getDriveStick().getTrigger(), () -> Strongback.submit(new ToggleGears(this)));
		reactor.onTriggered(getInput().getDriveStick().getButton(DRIVING_DISTANCE_BUTTON), () -> Strongback.submit(new DriveDistance(this, 50.0)));
		reactor.onTriggered(getInput().getDriveStick().getButton(TURNING_LEFT_BUTTON), () -> Strongback.submit(new TurnAngle(this, 10.0)));
		reactor.onTriggered(getInput().getDriveStick().getButton(TURNING_RIGHT_BUTTON), () -> Strongback.submit(new TurnAngle(this, -10.0)));
		reactor.onTriggered(getInput().getDriveStick().getButton(SHOOTER_ALIGN_BUTTON), () -> Strongback.submit(new ShooterAlign(this)));
	}
